$content = Get-Content 'client/src/components/home/ChooseTheLevel.jsx' -Raw
...
$openForms = [regex]::Matches($content, '<form[\s>]').Count
$closeForms = [regex]::Matches($content, '<\/form>').Count
...
Write-Output "Forms: $openForms / $closeForms"
