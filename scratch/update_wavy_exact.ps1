...
$json = $lines[$lines.Count - 1] | ConvertFrom-Json
$txt = $json.content

$wavyD = [regex]::Match($txt, 'd="([^"]+)"').Groups[1].Value

Write-Host "Extracted new wavy line d length: $($wavyD.Length)"

$jsxFile = "c:\react project\figma-akashvani\client\src\components\home\OffersSection.jsx"
$jsxContent = Get-Content $jsxFile -Raw

# Save exact new wavy line to scratch
$wavyD | Out-File "c:\react project\figma-akashvani\scratch\new_wavy_d.txt" -Encoding utf8

# Replace background wavy line path in OffersSection.jsx
# We find the first path in SVG (Background Wavy Line)
$pattern = '(?s)(\{/\* Background Wavy Line \*/\}\s*<path\s+d=")[^"]+("\s+fill="#F5F0EA"\s+fillOpacity=")[^"]+("\s*/>)'
$replacement = '$1' + $wavyD + '" fill="#F5F0EA" fillOpacity="0.5"/>'

$newJsx = [regex]::Replace($jsxContent, '(?s)(/\* Background Wavy Line \*/\s*<path\s+d=")[^"]+("\s+fill="#F5F0EA"\s+fillOpacity=")[^"]+("\s*/>)', "${1}${wavyD}${2}0.5${3}")

if ($newJsx.Length -eq $jsxContent.Length) {
    Write-Host "Regex match failed, doing direct string swap..."
}

# Let's rebuild build_final_offers.ps1 with the new wavy line
...
Write-Host "Updated OffersSection.jsx with exact new wavy line path and updated 730px viewBox coordinates!"
